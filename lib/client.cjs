if (typeof window !== "undefined") window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region src/client/store-module.ts
		...
		*/
		function loadStoreModule() {
			...
			const failures = [];
			for (const specifier of [CURRENT_STORE_MODULE, LEGACY_STORE_MODULE]) try {
...
`;
		...
		*/
		...
		//#endregion
		//#region src/client/credentials-face.ts
		...
		*/
		function createCredentialsFace() {
			let remote;
			let legacy;
			return {
				adoptRemoteNamespace(namespace) {
					const candidate = namespace;
					if (candidate !== void 0 && isFunction(candidate.describe) && isFunction(candidate.set)) remote = candidate;
				},
				adoptLegacyNamespace(namespace) {
					const candidate = namespace;
					if (candidate !== void 0 && isFunction(candidate.describe) && isFunction(candidate.set)) legacy = candidate;
				},
				async describe(ref) {
					if (remote !== void 0) {
					...
					if (legacy !== void 0) {
						...
						return result.ok ? result.value?.credentials?.[ref] : void 0;
					}
...
`;
		...
		*/
		function apply(ctx) {
			...
			credentials.adoptLegacyNamespace(connection?.api?.credentials);
			...
			ctx.effect(() => {
				const events = remote;
				const disposers = ["credentials/reference-updated", "credentials/updated"].map((event) => events.$on(event, (ref) => {
